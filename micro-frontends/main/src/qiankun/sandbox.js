        ...
        set(_, p, value) {
            if (sandboxRunning) {
                if (!rawWindow.hasOwnProperty(p)) {
                    ...
                    modifiedPropsOriginalValueMapInSandbox.set(
                        p,
                        originalValue
                    );
                }
    ...
    return {
        ...
        async mount() {
            ...
            if (sideEffectsRebuildersAtBootstrapping.length) {
                sideEffectsRebuildersAtBootstrapping.forEach((rebuild) =>
                    rebuild()
                );
            }
            ...
            if (!sandboxRunning) {
                currentUpdatedPropsValueMap.forEach((v, p) =>
                    setWindowProp(p, v)
                );
            }
        ...
        async unmount() {
            ...
            modifiedPropsOriginalValueMapInSandbox.forEach((v, p) =>
                setWindowProp(p, v)
            );
            addedPropsMapInSandbox.forEach((_, p) =>
