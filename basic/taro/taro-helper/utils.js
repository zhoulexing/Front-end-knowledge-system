...
export const isNodeModule = (filename) => NODE_MODULES_REG.test(filename)

export function isNpmPkg (name) {
  if (/^(\.|\/)/.test(name)) {
    return false
  }
  return true
...
export function isQuickAppPkg (name) {
  return /^@(system|service)\.[a-zA-Z]{1,}/.test(name)
}

export function isAliasPath (name, pathAlias) {
...
export function replaceAliasPath (filePath, name, pathAlias) {
...
export function promoteRelativePath (fPath) {
...
export function resolveStylePath (p) {
  const realPath = p
...
export function generateEnvList (env) {
  const res = { }
...
export function generateConstantsList (constants) {
  const res = { }
...
export function cssImports (content) {
  let match
  const results = []
...
export function emptyDirectory (dirPath, opts = { excludes: [] }) {
...
export const pascalCase =
  (str) => str.charAt(0).toUpperCase() + camelCase(str.substr(1))

export function getInstalledNpmPkgPath (pkgName, basedir) {
...
export function getInstalledNpmPkgVersion (pkgName, basedir) {
...
export function readDirWithFileTypes (floder) {
...
export function extnameExpRegOf (filePath) {
...
export function removeHeadSlash (str) {
  return str.replace(/^(\/|\\)/, '')
}
