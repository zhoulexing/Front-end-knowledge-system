...
export const makeConfig = async (buildConfig) => {
...
export const mergeOption = ([...options]) => {
...
export const processEnvOption = partial(mapKeys, (key) => `process.env.${key}`)
...
export const getModule = (appPath, {
  sourceDir,
  ...
  const cssModuleOptions = recursiveMerge({}, defaultCssModuleOption, postcssOption.cssModules)
  ...
  const urlOptions = recursiveMerge({}, defaultUrlOption, postcssOption.url)
...
export function getOutput (appPath, [{ outputRoot, publicPath, globalObject }, customOutput]) {
  return {
