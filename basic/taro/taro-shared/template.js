...
export class BaseTemplate {
    constructor() {
        ...
        this.modifyLoopBody;
        this.modifyLoopContainer;
        this.modifyTemplateResult;
    ...
    buildAttribute(attrs, nodeName) {
        return Object.keys(attrs)
            .map(
                (k) =>
                    `${k}="${
                        k.startsWith("bind") || k.startsWith("on")
                            ? attrs[k]
                    ...
                    if (hasOwn(component, prop)) {
                        ...
                        if (
                            ...
                            propValue = this.supportXS
                                ...
                                : `i.${toCamelCase(
                                      prop
                                  )}===undefined?${propValue}:i.${toCamelCase(
                                      prop
                                  )}`;
                        } else {
                            propValue = `i.${toCamelCase(prop)}||${
                                propValue || singleQuote("")
                            }`;
                        }
  ...
  <template name="tmpl_${level}_${comp.nodeName}_focus">
    <${comp.nodeName} ${this.buildAttribute(
            comp.attributes,
            comp.nodeName
        )} id="{{i.uid}}" />
  </template>
  ...
  `;
    ...
    buildStandardComponentTemplate(comp, level) {
        ...
        let child = this.supportXS
            ...
            : `<template is="tmpl_${nextLevel}_${
                  Shortcuts.Container
              }" data="{{${this.dataKeymap("i:item")}}}" />`;
    ...
    `;
        ...
        const nodeName =
            comp.nodeName === "slot" || comp.nodeName === "slot-view"
                ? "view"
                : comp.nodeName;

        let res = `
            <template name="tmpl_${level}_${comp.nodeName}">
                <${nodeName} ${this.buildAttribute(
            comp.attributes,
            comp.nodeName
        )} id="{{i.uid}}">${children}</${nodeName}>
            </template>
        `;
    ...
    buildPlainTextTemplate(level) {
        return `
            <template name="tmpl_${level}_#text" data="{{${this.dataKeymap(
            "i:i"
        )}}}">
        ...
        componentConfig.thirdPartyComponents.forEach((attrs, compName) => {
            template += `
                ...
                attrs
                    ...
                    <block ${Adapter.for}="{{i.${Shortcuts.Childnodes}}}" ${
                Adapter.key
            }="uid">
                        <template is="tmpl_${nextLevel}_${
                Shortcuts.Container
    ...
    buildContainerTemplate(level, restart = false) {
        ...
        if (restart) {
            ...
            const data = !this.isSupportRecursive
                ? `${this.dataKeymap(
                      `i:i,cid:${level},l:xs.f(l,i.${Shortcuts.NodeName})`
                  )}`
                : `${this.dataKeymap("i:i")}`;

            tmpl = this.supportXS
                ? `<template is="{{${xs}}}" data="{{${data}}}" />`
                : `<template is="{{'tmpl_${level}_' + i.${
                      Shortcuts.NodeName
                  }}}" data="{{${this.dataKeymap("i:i")}}}" />`;
        }
        return `
    ...
    buildPageTemplate = (baseTempPath) => {
        const template = `<import src="${baseTempPath}"/>
            <template is="taro_tmpl" data="{{${this.dataKeymap(
                "root:root"
            )}}}" />`;
...
export class UnRecursiveTemplate extends BaseTemplate {
    ...
    buildTemplate = (componentConfig) => {
        ...
        const components = Object.keys(this.miniComponents).filter((c) =>
            componentConfig.includes.size && !componentConfig.includeAll
    ...
    buildFloor(level, components, restart = false) {
        ...
        let template = components.reduce((current, nodeName) => {
            ...
            return (
                current +
                this.buildComponentTemplate({ nodeName, attributes }, level)
            );
        }, "");
